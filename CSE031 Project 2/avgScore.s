...
loop:
	...
	move $s4, $v0       # Move the value from register $v0 to register $s4
	div $s4, $s4, $s5   # Divide the value in $s4 by the value in $s5; store the quotient in $LO and remainder in $HI
	
	li $v0, 4           # Set syscall function code for print string
	la $a0, str5        # Load address of the string labeled str5 into $a0
	syscall             # Execute syscall to print string at address in $a0
	
	li $v0, 1           # Set syscall function code for print integer
	move $a0, $s4       # Move the quotient result from $s4 to $a0 for printing
	syscall             # Execute syscall to print integer in $a0
	
	lw $ra, 0($sp)      # Load the word at address $sp into $ra (restores return address)
	addi $sp, $sp, 4    # Increment stack pointer by 4 to clean up the stack
	li $v0, 10          # Set syscall function code for exit
